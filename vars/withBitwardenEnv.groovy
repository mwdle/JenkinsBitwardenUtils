// Calls the main withBitwarden function and automatically parses environment variables from secure note
def call(Map config, Closure body) {
    withBitwarden(config) { credential ->
        // Parse environment variables (Docker .env) from secure note -- Mimics behavior outlined in https://docs.docker.com/compose/how-tos/environment-variables/variable-interpolation/ (but not perfectly -- please let me know of any bugs you might find!)
        def envList = []
        if (credential.notes) {
